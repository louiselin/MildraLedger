...
var ContractLedger = function ( contract ) {
    ...
    this.contract.TransactionAddedEvent( {}, function ( err, data ) {
        self.onTransactionAdded( err, data );
    } );

    this.contract.WriteOffAddedEvent( {}, function ( err, data ) {
        self.onWriteOffEntityAdded( err, data );
    } );
...
ContractLedger.prototype.onTransactionAdded = function ( err, data ) {
    this.emit( 'transactionAdded', err, {
...
ContractLedger.prototype.onWriteOffEntityAdded = function ( err, data ) {
    this.emit( 'writeOffEntityAdded', err, {
        contractTxHash: data.transactionHash,
...
ContractLedger.prototype.addTransaction = function ( senderAddress, amount, txType ) {
    ...
    console.log( senderAddress, txId, timestamp, amount, txType, txHash );

    this.contract.addTransaction( txId, timestamp, amount, txType, txHash, this.txObject( senderAddress ) );
};
...
ContractLedger.prototype.addExpenseTransaction = function ( senderAddress, amount ) {
    this.addTransaction( senderAddress, amount, TxExpenses );
};
...
ContractLedger.prototype.addWriteOffEntity = function ( senderAddress, txId ) {
    ...
    this.contract.addWriteOffEntity( writeOffId, txId, timestamp, writeOffHash, this.txObject( senderAddress ) );
};
...
module.exports.TxExpenses = TxExpenses;
